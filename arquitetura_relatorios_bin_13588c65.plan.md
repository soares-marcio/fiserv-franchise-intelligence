---
name: Arquitetura relatorios BIN
overview: Aplicação de relatórios sobre o XLSX-template da carteira BIN, com vários master franqueados importando 1 a 2 arquivos por semana. Eixo CANAL > SUB-CANAL > CLIENTE (CNPJ), com EC como grão técnico de fato e identidade própria em UUID. Ruby 4.0.6, Rails 8.1.3.1, PostgreSQL via docker-compose (pgvector, web, worker Solid Queue, Metabase). Cada arquivo é um snapshot imutável, consolidado numa camada de verdade corrente.
todos:
  - id: stack-init
    content: Inicializar Rails 8.1.3.1 + Ruby 4.0.6 neste diretório, com docker-compose (Postgres+pgvector, Metabase, web, worker)
    status: pending
  - id: schema
    content: "Migrations: channels, sub_channels, companies, establishments, import_batches, import_templates, revenue_snapshots, map_snapshots, activation_proposals, daily_revenues, monthly_volumes, conversation_actions, map_snapshot_actions, raw_import_rows, data_anomalies"
    status: pending
  - id: pg-extensions
    content: "Primeira migration: plpgsql, pg_trgm, pgcrypto, unaccent, vector"
    status: pending
  - id: identidade
    content: UUID público nas cinco entidades endereçáveis, to_param por uuid, EC/CNPJ/external_id como referências externas
    status: pending
  - id: schema-consolidado
    content: "Camada corrente: daily_revenues_consolidated, monthly_volumes_consolidated, competencia_coverages por canal, daily_revenue_revisions"
    status: pending
  - id: duplicatas
    content: Detecção de EC duplicado (prefixo 3 com par 9) gravada em data_anomalies, sem fusão automática
    status: pending
  - id: anomalia-subcanal
    content: "Depois: tela do operador para visualizar CNPJ em mais de um SUB-CANAL (1 caso neste arquivo)"
    status: pending
  - id: template-1
    content: Registrar template 1 com cabeçalhos literais das 3 abas + normalização por aba (inversão razão/fantasia, CEP, telefone, datas, split do MELHOR CONVERSA)
    status: pending
  - id: import-job
    content: Job de import com dedup por checksum, validações de reconciliação, derivação do dia de corte e upsert na camada consolidada
    status: pending
  - id: audit
    content: Auditoria de faturamento por SUB-CANAL no grão CNPJ, com comparação dia-alinhada, alerta de cliente parado e visão semanal
    status: pending
  - id: manual-crud
    content: Entrada manual com as mesmas validações do template
    status: pending
  - id: analytics
    content: Views materializadas de auditoria, funil, concentração e aderência de segmento
    status: pending
  - id: escopo
    content: Ponto único de escopo na leitura (sem filtro hoje) e channel_id em toda view, preparando permissão por master sem autenticação agora
    status: pending
  - id: reports
    content: Telas Rails (Hotwire + Tailwind 4 + daisyUI 5) e exportação sobre as views
    status: pending
  - id: metabase
    content: Metabase apontado para as views por role PostgreSQL somente leitura
    status: pending
isProject: false
---
